...
import { analyticsApi, tripApi, driverMobileApi, maintenanceApi } from "@/lib/api";
...
import {
  ...
  ClipboardList, MapPin, Navigation, Calendar, Shield,
} from "lucide-react";
...
export default function DashboardHome() {
  ...
  const fetchMyRequests = useCallback(async () => {
    ...
    // Driver: also fetch assigned trips
    if (user?.role === "Driver") {
      try {
        const dRes = await driverMobileApi.assignments();
        setDriverAssignments(dRes.assignments || []);
      } catch (_) {
        setDriverAssignments([]);
      }
    }

    // Mechanic: also fetch maintenance schedules (read-only, fleet_user can read)
    if (user?.role === "Mechanic") {
      try {
        const mRes = await maintenanceApi.schedules();
        setMaintenanceSchedules(mRes.schedules || []);
      } catch (_) {
